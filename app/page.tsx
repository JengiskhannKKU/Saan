...
import { Search, ShoppingCart, User, Menu, Plus, ChevronLeft, ChevronRight} from "lucide-react"
...
export default function HomePage() {
  ...
  const getProductAtOffset = (offset: number) => {
    const index = (currentSlide + offset + featuredProducts.length) % featuredProducts.length
      ...
      <section className="py-8 px-4 bg-gradient-to-b from-green-50 to-gray-50">
        <div className="container mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-4 text-balance">สินค้าที่คุณอาจชอบ</h2>
            <p className="text-gray-600 text-pretty">สินค้าคัดสรรพิเศษเพื่อคุณโดยเฉพาะ</p>
          </div>
              ...
              <div className="flex items-center justify-center gap-8 py-8">
                ...
                <div className="hidden lg:block w-64 transition-all duration-700 ease-in-out">
                  ...
                  <div className="text-center mt-4 opacity-60 transition-opacity duration-700">
                    <h4 className="font-medium text-sm text-gray-800">{getProductAtOffset(-1).name}</h4>
                    <p className="text-xs text-gray-600">฿{getProductAtOffset(-1).price}</p>
                  </div>
                </div>
                ...
                <div className="transition-all duration-700 ease-in-out transform">
                  <Card className="w-80 shadow-2xl border-0 overflow-hidden transform hover:scale-105 transition-transform duration-300 bg-white">
                    <div className="relative aspect-square">
                      <Image
                        src={getProductAtOffset(0).image || "/placeholder.svg"}
                    ...
                    <CardContent className="p-6 text-center">
                      <div className="flex items-center justify-center gap-1 mb-2">
                        {[...Array(5)].map((_, i) => (
                          <span
                            key={i}
                            className={`text-sm ${i < Math.floor(getProductAtOffset(0).rating) ? "text-yellow-400" : "text-gray-300"}`}
                          >
                            ★
                          </span>
                        ))}
                        <span className="text-sm text-gray-500 ml-1">({getProductAtOffset(0).rating})</span>
                      </div>
                      <h3 className="text-xl font-bold text-gray-800 mb-2">{getProductAtOffset(0).name}</h3>
                      <p className="text-gray-600 mb-4 text-sm">{getProductAtOffset(0).description}</p>
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="text-2xl font-bold text-green-600">฿{getProductAtOffset(0).price}</span>
                          <p className="text-xs text-gray-500">{getProductAtOffset(0).sold}</p>
                        </div>
              ...
              {featuredProducts.map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 hover:scale-125 ${
                    index === currentSlide ? "bg-green-500 scale-110" : "bg-gray-300 hover:bg-gray-400"
                  }`}
                  onClick={() => {
                    if (!isTransitioning) {
                      setIsTransitioning(true)
